...
const WORLD1_LESSONS = [
  {
    ...
    steps: [
      { g: "Press ▶ Run and make the computer say its first line.",
        ok: (c) => c.outs.length >= 1,
        yay: "The computer spoke — your first command, obeyed.",
        miss: "One print line, words inside quotes — then ▶ Run." },
      { g: "Add a SECOND print line under it — a new sentence of yours. Run again.",
        ok: (c) => c.outs.length >= 2,
        yay: "Two lines — top to bottom, in YOUR order.",
        miss: "A whole new print(\"...\") line under the first one." },
      { g: "One more — make it three.",
        ok: (c) => c.outs.length >= 3,
        yay: "Three commands, three obediences.",
        miss: "Three print lines total — each on its own line." },
    ],
  ...
  },
  {
    ...
    starter: `forward(120)\nright(90)\nforward(120)\n`,
    steps: [
      { g: "▶ Run — watch the turtle obey, top to bottom.",
        ok: (c) => c.lines.length >= 2,
        yay: "It walked and turned, exactly as written.",
        miss: "Keep the forward and right lines — then press Run." },
      { g: "Add right(90) then forward(120) at the bottom. Run.",
        ok: (c) => c.lines.length >= 3,
        yay: "A third stroke — the shape is growing.",
        miss: "Two new lines at the end: right(90), then forward(120)." },
      { g: "One more turn and one more walk — four strokes total.",
        ok: (c) => c.lines.length >= 4,
        yay: "Four strokes, steered line by line by YOU.",
        miss: "Add another right(...) and another forward(...)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — three sides, three colors.",
        ok: (c) => c.lines.length >= 3,
        yay: "The colored triangle appears.",
        miss: "Keep the three forward lines — press Run." },
      { g: "Make it YOURS: change any color word to one you love. Run.",
        ok: (c) => c.lines.length >= 3 && c.code !== c.starter,
        yay: "Your palette now — the pen obeys your taste.",
        miss: "Swap one color name inside the quotes — try \"orchid\" or \"teal\"." },
    ],
  ...
  },
  {
    ...
    starter: `size = 100\n\nforward(size)\nright(90)\nforward(size)\nright(90)\nforward(size)\nright(90)\nforward(size)\n`,
    steps: [
      { g: "▶ Run — one jar feeds all four sides.",
        ok: (c) => c.lines.length >= 4,
        yay: "A perfect square, fed from one jar.",
        miss: "Keep the four forward(size) lines — press Run." },
      { g: "Change ONLY the first line: size = 180. Run again.",
        ok: (c) => c.lines.length >= 4 && /size\s*=\s*(?!100\b)\d+/.test(c.code),
        yay: "One number changed — the WHOLE square obeyed.",
        miss: "Touch only the top line: size = 180." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run the three-line spiral.",
        ok: (c) => c.lines.length >= 8,
        yay: "Three lines of code — a whole galaxy.",
        miss: "Keep the loop exactly as it is and press Run." },
      { g: "Change right(100) to right(121). Run — a new galaxy.",
        ok: (c) => c.lines.length >= 8 && c.code !== c.starter,
        yay: "One number, a different universe. That's the experiment habit.",
        miss: "Change one number — the angle, the range, or the * 4." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "Write the loop line: for i in range(5): — then Run.",
        ok: (c) => /for\s+\w+\s+in\s+range\s*\(\s*5\s*\)\s*:/.test(c.code),
        yay: "The five-times spell is cast.",
        miss: "Exactly: for i in range(5): — don't forget the colon." },
      { g: "Inside it, 4 spaces deep: forward(150) then right(144). Run.",
        ok: (c) => c.lines.length >= 5,
        yay: "Five strokes drawn!",
        miss: "Two indented lines inside the loop: forward(150), right(144)." },
      { g: "Perfect it — equal lines, every turn exactly 144.",
        ok: (c) => c.lines.length === 5,
        yay: "A clean five — now the geometry judges.",
        miss: "Exactly 5 strokes — no extras (penup() can hide travel)." },
    ],
...
const WORLD2_LESSONS = [
  {
    ...
    steps: [
      { g: "▶ Run — and answer the question when it pops up!",
        ok: (c) => c.outs.length >= 1 && /input\(/.test(c.code),
        yay: "It listened — and used YOUR answer.",
        miss: "Press Run, type an answer in the popup, press OK." },
      { g: "Add a second question into a new jar: food = input(\"Favorite food?\"). Run.",
        ok: (c) => (c.code.match(/input\s*\(/g) || []).length >= 2,
        yay: "Two questions asked.",
        miss: "A new line: food = input(\"Favorite food?\")" },
      { g: "Print a reply that uses the second answer too.",
        ok: (c) => (c.code.match(/input\s*\(/g) || []).length >= 2 && (c.code.match(/print\s*\(/g) || []).length >= 3,
        yay: "A whole conversation — both answers, used.",
        miss: "print(\"Yummy, \" + food + \"!\")" },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — and answer exactly: blue",
        ok: (c) => c.lines.length >= 3,
        yay: "A road was chosen, and it drew.",
        miss: "Run, type blue in the popup, press OK." },
      { g: "Run AGAIN — answer anything else. Watch the other road.",
        ok: (c) => c.lines.length >= 3 && c.runs >= 2,
        yay: "Both roads visited — one decision, two worlds.",
        miss: "Press ▶ Run once more, with a different answer." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — meet your star.",
        ok: (c) => c.lines.length >= 3,
        yay: "A star of chance.",
        miss: "Press ▶ Run." },
      { g: "Run AGAIN. Is it the same star?",
        ok: (c) => c.lines.length >= 3 && c.runs >= 2,
        yay: "Different! The dice threw again.",
        miss: "Just press ▶ Run once more — and compare." },
      { g: "Add two colors you love into the bag. Run.",
        ok: (c) => (c.code.match(/"[a-zA-Z]+"/g) || []).length >= 6,
        yay: "A six-color bag — a proper surprise.",
        miss: "Two more \"colors\" inside the [ ], with commas between." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — ask the fortune teller something!",
        ok: (c) => c.outs.length >= 2,
        yay: "Destiny has spoken.",
        miss: "Run, type a question in the popup, press OK." },
      { g: "Write four MORE fortunes in your own voice. Run and ask again.",
        ok: (c) => (c.code.match(/"[^"]{2,}"/g) || []).length >= 8,
        yay: "Eight destinies strong — a real oracle.",
        miss: "Each fortune: a quoted line inside the [ ], comma after it." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — play one round. Win or learn!",
        ok: (c) => c.outs.length >= 1,
        yay: "One round played — the machine judged fairly.",
        miss: "Run, and type a number guess in the popup." },
      { g: "Make it harder: change randint(1, 10) to randint(1, 20).",
        ok: (c) => /randint\(\s*1\s*,\s*20\s*\)/.test(c.code),
        yay: "A 1-to-20 arena. Braver game.",
        miss: "Find randint(1, 10) at the top — make the 10 a 20." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "First road: if mood == \"happy\": with drawing lines inside. Run, type happy.",
        ok: (c) => /if\s+mood\s*==/.test(c.code) && (c.lines.length >= 1 || c.outs.length >= 1),
        yay: "The first feeling has its art.",
        miss: "if mood == \"happy\": then indented drawing lines under it." },
      { g: "Second road: elif mood == \"sleepy\": with DIFFERENT art.",
        ok: (c) => /\belif\b/.test(c.code),
        yay: "Two moods, two arts.",
        miss: "elif mood == \"sleepy\": — with its own indented lines." },
      { g: "Last road: else: — art for every other feeling.",
        ok: (c) => /^\s*else\s*:/m.test(c.code),
        yay: "No feeling left without art.",
        miss: "else: at the same depth as if — its lines indented under it." },
    ],
...
const WORLD3_LESSONS = [
  {
    ...
    steps: [
      { g: "▶ Run — six hexagons from four little lines.",
        ok: (c) => c.lines.length >= 12,
        yay: "The nest worked: thirty-six moves.",
        miss: "Keep both loops as they are — press Run." },
      { g: "Change BOTH range numbers and the last right(). Hunt a new ring.",
        ok: (c) => c.lines.length >= 12 && c.code !== c.starter,
        yay: "A ring nobody drew before you.",
        miss: "Try 8 shapes of 4 sides — and a final right(45)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — twelve squares bloom into a rose window.",
        ok: (c) => c.lines.length >= 24,
        yay: "Centuries of geometry, computed in a blink.",
        miss: "Keep the nest — press Run." },
      { g: "Craft a NEW tile: 8 squares with right(45) — or 10 with right(36). Run.",
        ok: (c) => (/range\(\s*8\s*\)/.test(c.code) && /right\(\s*45\s*\)/.test(c.code)) ||
                   (/range\(\s*10\s*\)/.test(c.code) && /right\(\s*36\s*\)/.test(c.code)),
        yay: "A new window — you own the 360 recipe now.",
        miss: "Two numbers change together: range(8) + right(45), or range(10) + right(36)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — a family of squares, from one counting line.",
        ok: (c) => c.lines.length >= 16,
        yay: "Six sizes, one range.",
        miss: "Keep the three-number range — press Run." },
      { g: "Make it count DOWN: range(120, 19, -20). Does the drawing change?",
        ok: (c) => /range\(\s*120\s*,\s*19\s*,\s*-20\s*\)/.test(c.code),
        yay: "Same squares, reverse order — the drawing kept the secret.",
        miss: "Exactly: range(120, 19, -20) — the minus does the counting down." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — six petals, then it stops itself.",
        ok: (c) => c.lines.length >= 10,
        yay: "The promise was kept — the loop knew when to stop.",
        miss: "Keep the while and its counter — press Run." },
      { g: "Grow the flower: while n < 12, and the turn becomes 30. Run.",
        ok: (c) => /while\s+\w+\s*<\s*12/.test(c.code) && /right\(\s*30\s*\)/.test(c.code),
        yay: "Twelve petals — condition and angle, agreeing at 360.",
        miss: "BOTH numbers change: < 12 in the while, right(30) inside." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — light the sky over the Union.",
        ok: (c) => c.lines.length >= 60,
        yay: "Eight rockets, commanded by a nest.",
        miss: "Keep both loops — press Run." },
      { g: "A bigger show: make the outer loop range(15). Run.",
        ok: (c) => /range\(\s*15\s*\)/.test(c.code) && c.lines.length >= 60,
        yay: "Fifteen rockets — the sky fills.",
        miss: "The OUTER range counts the rockets: range(15)." },
      { g: "A gold dot() at each rocket's heart — after the inner loop ends.",
        ok: (c) => (c.cmds || []).some(x => x.t === "dot"),
        yay: "Hearts of gold in every burst. Eid Al Etihad!",
        miss: "After the inner loop (4-space indent): color(\"gold\") then dot(6)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "Choose your N — write the outer loop: for i in range(8):  (or 6, 10, 12).",
        ok: (c) => /for\s+\w+\s+in\s+range\(\s*(6|8|10|12)\s*\)/.test(c.code),
        yay: "The wheel is chosen.",
        miss: "One line: for i in range(8): — with the colon." },
      { g: "Inside it, deeper indent: your shape's own loop. Run — see it draw.",
        ok: (c) => c.lines.length >= 8,
        yay: "A shape is being repeated — the nest breathes.",
        miss: "An inner for with forward(...) and right(...) — eight spaces deep." },
      { g: "Close the ring: after the inner loop, right(360 ÷ your N). Grow past 18 lines.",
        ok: (c) => c.lines.length >= 18,
        yay: "The ring closes — geometry obeys you.",
        miss: "The last right() at 4 spaces: 45 for N=8, 60 for N=6, 30 for N=12." },
    ],
...
const WORLD4_LESSONS = [
  {
    ...
    starter: `def star():\n    for i in range(5):\n        forward(120)\n        right(144)\n\ncolor("gold")\nstar()\npenup()\njump(-150, -60)\npendown()\ncolor("crimson")\nstar()\n`,
    steps: [
      { g: "▶ Run — one taught word, said twice: twin stars.",
        ok: (c) => c.lines.length >= 8 && /def\s+\w+\s*\(/.test(c.code),
        yay: "Teach once, say forever.",
        miss: "Keep def star(): and both star() calls — press Run." },
      { g: "Teach a SECOND word — def burst(): with its own recipe. Say it below. Run.",
        ok: (c) => (c.code.match(/def\s+\w+\s*\(/g) || []).length >= 2,
        yay: "Python's dictionary grew twice today.",
        miss: "A new def block at the top, its recipe indented — then its name() below." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — one word, three gifts, three squares.",
        ok: (c) => c.lines.length >= 12,
        yay: "The door works — each call hands in a size.",
        miss: "Keep the three square(...) calls — press Run." },
      { g: "Add TWO more calls with new sizes. Run.",
        ok: (c) => (c.code.match(/square\(\s*\d+\s*\)/g) || []).length >= 5,
        yay: "Five squares from ONE recipe.",
        miss: "Two more lines like square(80) — any sizes you like." },
      { g: "Slip a color(\"...\") between two calls — paint one square differently.",
        ok: (c) => (c.code.match(/color\(/g) || []).length >= 2,
        yay: "A painted family — words and style, composed.",
        miss: "A color(\"crimson\") line between two square calls." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — triangle, square, hexagon, octagon: one word rules them.",
        ok: (c) => c.lines.length >= 15,
        yay: "The universal shape word works.",
        miss: "Keep the four poly(...) calls — press Run." },
      { g: "Call poly(12, 45) and poly(20, 30) — watch shapes become circles.",
        ok: (c) => /poly\(\s*12\s*,/.test(c.code) && /poly\(\s*20\s*,/.test(c.code),
        yay: "More sides, smoother ring — the circle is a polygon's dream.",
        miss: "Two more calls at the bottom: poly(12, 45) and poly(20, 30)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — squares that double, fed by an answering word.",
        ok: (c) => c.lines.length >= 12,
        yay: "The answer came back, and the jar caught it.",
        miss: "Keep double(n) and the three squares — press Run." },
      { g: "Teach triple(n): return n * 3. Grow a second family with it. Run.",
        ok: (c) => /def\s+triple\s*\(/.test(c.code) && /triple\(/.test(c.code.split("def")[c.code.split("def").length - 1]),
        yay: "Two answering words — growth at your command.",
        miss: "def triple(n): return n * 3 — then s = triple(s) between squares." },
    ],
  ...
  },
  {
    ...
    starter: `def ray():\n    forward(75)\n    back(75)\n\ndef sun():\n    for i in range(12):\n        ray()\n        right(30)\n\ncolor("gold")\nwidth(4)\nsun()\npenup()\njump(150, 90)\npendown()\ncolor("darkorange")\nsun()\n`,
    steps: [
      { g: "▶ Run — ray builds sun; two suns rise.",
        ok: (c) => c.lines.length >= 20,
        yay: "Words made of words — the tower stands.",
        miss: "Keep ray(), sun() and both calls — press Run." },
      { g: "Teach sky(): jump somewhere, say sun() — three times inside. Then below: just sky().",
        ok: (c) => /def\s+sky\s*\(/.test(c.code) && (c.code.match(/sun\(\)/g) || []).length >= 4,
        yay: "ray → sun → sky. You built software's whole idea.",
        miss: "def sky(): with three penup/jump/pendown/sun() groups inside." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "Teach your first word — def frame(): with drawing inside. Say it below. Run.",
        ok: (c) => (c.code.match(/def\s+\w+\s*\(/g) || []).length >= 1 && c.lines.length >= 4,
        yay: "The card has its first word.",
        miss: "def frame(): with indented forward/right lines — then frame() below." },
      { g: "Teach a second word — star() perhaps — and say it TWICE, scattered.",
        ok: (c) => (c.code.match(/def\s+\w+\s*\(/g) || []).length >= 2 && c.lines.length >= 10,
        yay: "A vocabulary of two, and a card getting rich.",
        miss: "A second def block — then say it twice: jump somewhere between the calls." },
      { g: "Finish the gift: print the Eid greeting using the name.",
        ok: (c) => c.outs.length >= 1 && /print\(/.test(c.code),
        yay: "A machine that makes gifts — signed by its maker.",
        miss: "print(\"Eid Mubarak, \" + name + \"!\")" },
    ],
...
const WORLD5_LESSONS = [
  {
    ...
    steps: [
      { g: "▶ Run — three treasures, opened by slot number.",
        ok: (c) => (c.cmds || []).filter(x => x.t === "dot").length >= 3,
        yay: "Slots 0, 1, 2 — opened.",
        miss: "Keep the three jump + dot pairs — press Run." },
      { g: "Add TWO treasures to the box — and two more jump + dot pairs to show them.",
        ok: (c) => (c.cmds || []).filter(x => x.t === "dot").length >= 5,
        yay: "Five treasures — and the fourth lived in slot 3, didn't it?",
        miss: "Two \"colors\" into the box, then jump(...), color(colors[3]), dot(30)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — the loop visits every treasure, no numbers needed.",
        ok: (c) => c.lines.length >= 10,
        yay: "Five rays, five colors — one visiting loop.",
        miss: "Keep for c in colors: — press Run." },
      { g: "Grow the box to 8 colors — and fix the turn to right(45). Run.",
        ok: (c) => (c.code.match(/"[a-zA-Z]+"/g) || []).length >= 8 && /right\(\s*45\s*\)/.test(c.code),
        yay: "Eight treasures, eight rays — the loop just followed.",
        miss: "Three more \"colors\" in the box, and the turn becomes 45 (360 ÷ 8)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — welcome three guests to the majlis.",
        ok: (c) => c.outs.length >= 4,
        yay: "The box grew, the count was true, every guest greeted.",
        miss: "Run and type three names into the popups." },
      { g: "Run AGAIN — enter the SAME name twice. Watch the door-keeper.",
        ok: (c) => c.outs.length >= 4 && c.runs >= 2,
        yay: "The in check caught the double — no guest counted twice.",
        miss: "Press Run once more; give one name two times." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — your name, walked letter by letter.",
        ok: (c) => c.outs.length >= 3,
        yay: "First letter, length, and a dotted trail — words are boxes.",
        miss: "Run and type your name in the popup." },
      { g: "Print the LAST letter too: name[len(name) - 1]. Run.",
        ok: (c) => /len\(\s*name\s*\)\s*-\s*1/.test(c.code),
        yay: "Minus one — because the zero shifted everything. You've got it.",
        miss: "A new line: print(\"Last: \" + name[len(name) - 1])" },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — whisper a message, watch it hide in plain sight.",
        ok: (c) => c.outs.length >= 2,
        yay: "Encoded — the oldest trick, by your hand.",
        miss: "Run, type a small-letters message in the popup." },
      { g: "Build the DECODER below: slide spot - 3, and if spot < 0: spot = spot + 26.",
        ok: (c) => /-\s*3/.test(c.code) && /\+\s*26/.test(c.code),
        yay: "Encode, decode — the circle turns both ways. Test it honestly!",
        miss: "Same machine, reversed: spot - 3, and below zero wrap with + 26." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "Grow BOTH boxes to 3 pairs — question and answer-twin, same slots. Run the quiz.",
        ok: (c) => (c.code.match(/\[[^\]\n]*,[^\]\n]*\]/g) || []).length >= 2,
        yay: "Three pairs, twin by number — and the loop followed alone.",
        miss: "Two more entries in questions, their twins in answers — same order." },
      { g: "Add the kind verdict: if score == len(questions): praise — else: encourage.",
        ok: (c) => /if\s+score\s*==\s*len\(/.test(c.code),
        yay: "A quiz that judges kindly — like a good teacher.",
        miss: "After the loop: if score == len(questions): print praise, else: print warmth." },
    ],
...
const WORLD6_LESSONS = [
  {
    ...
    starter: `game.x = -200\n\ndef tick():\n    game.x = game.x + 3\n    if game.x > 220:\n        game.x = -220\n    color("peru")\n    penup()\n    jump(game.x, -130)\n    dot(16)\n    color("saddlebrown")\n    jump(game.x + 11, -123)\n    dot(5)\n`,
    steps: [
      { g: "▶ Run — your falcon glides, forever. (Esc stops it.)",
        ok: (c) => (c.frames || []).length >= 60,
        yay: "It LIVES — thirty blinks a second.",
        miss: "Keep tick() as it is — press Run and watch." },
      { g: "Make it glide the OTHER way: minus the speed, wrap on the LEFT edge.",
        ok: (c) => /game\.x\s*-\s*\d/.test(c.code) && /if\s+game\.x\s*<\s*-\d+/.test(c.code),
        yay: "Westward falcon — speed, sign, and wrap, all yours.",
        miss: "game.x = game.x - 3, and: if game.x < -220: game.x = 220" },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — then FLY it with the ← → arrow keys.",
        ok: (c) => (c.frames || []).length >= 60,
        yay: "The player is inside the world now.",
        miss: "Press Run, then hold an arrow key — the falcon obeys YOU." },
      { g: "Give it vertical life: a game.y jar, up and down keys, jump uses game.y.",
        ok: (c) => /key_pressed\(\s*"up"\s*\)/.test(c.code) && /game\.y/.test(c.code),
        yay: "Four directions — a whole sky to command.",
        miss: "game.y = -130 at top; two more ifs for \"up\"/\"down\"; jump(game.x, game.y)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — chase the falling houbara. (You can't catch it YET.)",
        ok: (c) => (c.frames || []).length >= 60,
        yay: "Two lives in one sky, each with its own jars.",
        miss: "Press Run — watch it fall, respawn, fall again." },
      { g: "Make the sky busier: a faster fall, or a second prey with its own jars.",
        ok: (c) => c.code !== c.starter && (c.frames || []).length >= 60,
        yay: "The sky obeys its maker.",
        miss: "Change the - 6 fall speed — or add game.prey2_x / game.prey2_y jars." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — HUNT! Catch your first houbara with the arrows.",
        ok: (c) => (c.frames || []).some(f => f.some(x => x.t === "text")),
        yay: "Judge, memory, announcement — the game is WHOLE.",
        miss: "Press Run, steer under the falling prey — the score writes itself." },
      { g: "Tune the talons: change the < 30 catch-circle. Kinder or crueler — your justice.",
        ok: (c) => /distance\([^)]*\)\s*<\s*(?!30\b)\d+/.test(c.code),
        yay: "Game design is empathy — and you just practiced it.",
        miss: "Find distance(...) < 30 — try < 45 (kind) or < 22 (cruel)." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — play to score 5. Feel the storm build with every catch.",
        ok: (c) => (c.frames || []).length >= 60,
        yay: "Edges held, challenge grew, mercy stayed.",
        miss: "Press Run and hunt — the speed climbs as you score." },
      { g: "Tune YOUR game: wingspeed, catch-circle, or the speed cap. Make it yours.",
        ok: (c) => c.code !== c.starter && (c.frames || []).length >= 60,
        yay: "Designed, not just built. Save it to your Gallery, maker.",
        miss: "Change one number: the 7 wingspeed, the < 34 circle, or the 14 cap." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "The heartbeat first: game jars at top, def tick(): — draw your player inside. Run.",
        ok: (c) => /def\s+tick\s*\(/.test(c.code) && ((c.frames || [])[10] || []).length >= 1,
        yay: "Your world breathes.",
        miss: "game.x = 0 at top, then def tick(): with jump + dot inside." },
      { g: "Let the player in: key_pressed(...) moves your game.x.",
        ok: (c) => /key_pressed\(/.test(c.code),
        yay: "It answers to human hands now.",
        miss: "Inside tick: if key_pressed(\"left\"): game.x = game.x - 6 (and right)." },
      { g: "The judge and the sky-score: distance(...) decides, write(...) announces.",
        ok: (c) => /(?<![\w])distance\(/.test(c.code) && /(?<![\w])write\(/.test(c.code),
        yay: "Four organs beating — now add the rule nobody taught you.",
        miss: "A distance(...) < N if — and write(-230, 200, \"Score: \" + str(...))." },
    ],
...
const WORLD7_LESSONS = [
  {
    ...
    steps: [
      { g: "▶ Run — five numbers become five bars.",
        ok: (c) => c.lines.length >= 4,
        yay: "A chart is just drawing, driven by data. Proven.",
        miss: "Keep the loop and the box — press Run." },
      { g: "Chart YOUR week: put your real numbers in the box. Add a title write at the top.",
        ok: (c) => c.code !== c.starter && (c.cmds || []).filter(x => x.t === "text").length >= 5,
        yay: "Your data, your bars, your title — an honest chart by hand.",
        miss: "Change the numbers in goals, and add write(-120, 200, \"My week\")." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — Dubai's year, asked by NAME.",
        ok: (c) => c.lines.length >= 3,
        yay: "Named data, charted — the W5 promise, kept.",
        miss: "Keep the dictionary and its loop — press Run." },
      { g: "Pack YOUR dictionary — foods and scores — and chart it with the same loop.",
        ok: (c) => c.code !== c.starter && /\{[^{}\n]*:/.test(c.code) && c.lines.length >= 3,
        yay: "Your names, your numbers — the loop didn't change at all.",
        miss: "votes = {\"machboos\": 9, \"luqaimat\": 10, ...} — same loop below." },
    ],
    ...
    check: (ctx) => {
      ...
      if (ctx.lines.length < 3) return { pass: false, msg: "Chart it — a bar per entry, at least three." };
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run the year. (First run downloads the toolbox — wait it out.)",
        ok: (c) => !!c.chart,
        yay: "A research-grade chart, from your hand.",
        miss: "Keep the four sacred lines — press Run and be patient once." },
      { g: "Plot a SECOND line before show: night temps, your guesses. Run.",
        ok: (c) => (c.code.match(/plt\.plot\(/g) || []).length >= 2 && !!c.chart,
        yay: "Two lines, one chart — day and night, compared.",
        miss: "A second list, then plt.plot(months, nights) BEFORE plt.show()." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run the rain — titled, labeled, honest.",
        ok: (c) => !!c.chart,
        yay: "This chart could walk into a lab meeting.",
        miss: "Keep title, xlabel, ylabel — press Run." },
      { g: "Swap in a dataset YOU love — keep all three labels. No rumors.",
        ok: (c) => c.code !== c.starter && /plt\.title\(/.test(c.code) && /plt\.xlabel\(/.test(c.code) && /plt\.ylabel\(/.test(c.code) && !!c.chart,
        yay: "Your data, fully labeled — the law is in your hands now.",
        miss: "Change months/rain to your own — but the three labels stay, always." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — raw text walks in, a labeled chart walks out.",
        ok: (c) => !!c.chart,
        yay: "Five cities, tamed by split.",
        miss: "Keep the pipeline — press Run." },
      { g: "ADD two more cities into the raw text. Run — nothing else changes. Feel why.",
        ok: (c) => c.code !== c.starter && !!c.chart,
        yay: "The parser followed alone — pipelines scale themselves.",
        miss: "Inside the text: \\nRas Al Khaimah,40 — the loop does the rest." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "Your real week: a numbers box for the seven days, plt.bar it. Run.",
        ok: (c) => /plt\.(bar|plot)\(/.test(c.code) && !!c.chart,
        yay: "Real life, charted.",
        miss: "pages = [your seven real numbers], then plt.bar(days, pages), plt.show()." },
      { g: "The law: plt.title, plt.xlabel, plt.ylabel — all three.",
        ok: (c) => /plt\.title\(/.test(c.code) && /plt\.xlabel\(/.test(c.code) && /plt\.ylabel\(/.test(c.code) && !!c.chart,
        yay: "No rumors — your life deserves labels.",
        miss: "Three lines before show: title, xlabel, ylabel." },
      { g: "The scientist's finish: print ONE honest sentence the chart taught you.",
        ok: (c) => c.outs.length >= 1,
        yay: "Collected, charted, labeled, and READ. That's evidence thinking.",
        miss: "print(\"My ... doubles on weekends.\") — say what the SHAPE says." },
    ],
...
const WORLD8_LESSONS = [
  {
    ...
    steps: [
      { g: "▶ Run — crown the champion. The gold bar is the winner.",
        ok: (c) => c.outs.length >= 1 && c.lines.length >= 4,
        yay: "Your first named algorithm ran.",
        miss: "Keep the fight loop — press Run." },
      { g: "Flip the recipe: find the SMALLEST. One character changes. Run.",
        ok: (c) => /if\s+\w+\s*<\s*champion/.test(c.code),
        yay: "< instead of > — the whole recipe reversed. Patterns travel.",
        miss: "In the fight: if n < champion — the smallest survives." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — hunt Sara. Read the receipt.",
        ok: (c) => c.outs.length >= 1,
        yay: "Found — and the cost was counted.",
        miss: "Keep the hunt loop — press Run." },
      { g: "Experiment: hunt someone ABSENT (target = \"Hessa\") — or move \"Sara\" to slot 0. Run.",
        ok: (c) => /target\s*=\s*"(?!Sara")/.test(c.code) || /\[\s*"Sara"/.test(c.code),
        yay: "The receipt told the truth — position is luck, absence costs the most.",
        miss: "Change target to \"Hessa\", or make the box start with \"Sara\"." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — chaos in, staircase out.",
        ok: (c) => c.outs.length >= 1 && c.lines.length >= 4,
        yay: "Order from chaos, by your own hand.",
        miss: "Keep the sorter — press Run." },
      { g: "Feed it YOUR numbers — any chaos. The staircase must still rise. Run.",
        ok: (c) => c.code !== c.starter && c.lines.length >= 4,
        yay: "Any chaos, same staircase — that's what ALGORITHM means.",
        miss: "Change the numbers box at the top to numbers you choose." },
      { g: "One character: hunt the LARGEST each round. The staircase flips.",
        ok: (c) => /if\s+\w+\s*>\s*smallest/.test(c.code),
        yay: "Biggest-first — you just bent a classic to your will.",
        miss: "In the hunt: if n > smallest (yes, the jar name now lies — rename it if it bothers you!)." },
    ],
    ...
    check: (ctx) => {
      ...
      for (let i = 1; i < bars.length; i++) {
        if (bars[i] < bars[i - 1] - 1) rising = false;
        if (bars[i] > bars[i - 1] + 1) falling = false;
      }
      if (!rising && !falling) return { pass: false, msg: "The staircase betrays you — the bars must only ever rise (or only ever fall). Check the sort." };
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run the measurement — three hunts, three receipts, one chart.",
        ok: (c) => c.outs.length >= 3 && !!c.chart,
        yay: "You just measured an algorithm's cost.",
        miss: "Keep the experiment — press Run." },
      { g: "Add a box of 40: one number in sizes, one label in the bar list. Predict first!",
        ok: (c) => /sizes\s*=\s*\[[^\]]*40/.test(c.code) && !!c.chart,
        yay: "Forty names, forty steps — was your prediction right?",
        miss: "sizes = [5, 10, 20, 40] and a \"40 names\" label in plt.bar." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "▶ Run — teach the machine, then read its three judgments.",
        ok: (c) => c.outs.length >= 3 && !!c.chart,
        yay: "fit studied, predict judged — you taught a machine.",
        miss: "Keep fit and predict — press Run (the big download, once)." },
      { g: "Try a giant mystery fruit: add [60, 20] to the mystery list. Run.",
        ok: (c) => /\[\s*60\s*,\s*20\s*\]/.test(c.code),
        yay: "It answered CONFIDENTLY about nonsense — THAT is the lesson about AI.",
        miss: "mystery = [[39, 8], [21, 5], [30, 6], [60, 20]] — then Run." },
    ],
  ...
  },
  {
    ...
    steps: [
      { g: "Teach your first two def words — the capstone speaks YOUR vocabulary. Run.",
        ok: (c) => (c.code.match(/def\s+\w+\s*\(/g) || []).length >= 2,
        yay: "Two words of your own — the foundation stands.",
        miss: "Two def blocks — start from a thing you already love and grow it." },
      { g: "Mix worlds: bring in at least two big powers — charts, input, random, dicts, split, ML.",
        ok: (c) => [/plt\./, /input\(/, /def\s+tick/, /\{[^{}\n]*:/, /\.split\(/, /random\./, /\.fit\(/].filter(rx => rx.test(c.code)).length >= 2,
        yay: "Worlds are mixing — this is a REAL program now.",
        miss: "Add a chart of your results, an input question, or a random twist." },
      { g: "Sign it: print(\"Built by NAME — Barmij, Worlds 1 to 8.\") — you earned the line.",
        ok: (c) => /print\(|write\(/.test(c.code) && (c.outs.length >= 1 || (c.cmds || []).length >= 1 || !!c.chart),
        yay: "Signed. Sealed. Yours.",
        miss: "The last line: your printed signature." },
    ],
